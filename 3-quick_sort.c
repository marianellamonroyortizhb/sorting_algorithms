...
void quick_sort(int *array, size_t size)
{
	if (size < 2)
		return;
	quick_sort_funct(array, size, 0, size - 1);

}

/**
 * quick_sort_funct - using quick sort method
 *@array: unsorted array of integers
 *@size: size of the array
 *@p_low: low position
...
void quick_sort_funct(int *array, size_t size, int p_low, int p_high)
{
	...
	if (p_low < p_high)
	{
		...
		quick_sort_funct(array, size, p_low, pi - 1);
		quick_sort_funct(array, size, pi + 1, p_high);
	}
}
